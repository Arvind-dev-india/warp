...
impl ChannelState {
    ...
    pub fn is_telemetry_available() -> bool {
        // [FORK] Honor the WARP_DISABLE_TELEMETRY env-var kill switch.
        if env_overrides::telemetry_disabled() {
            return false;
        }
    ...
    pub fn rudderstack_non_ugc_destination() -> RudderStackDestination {
        ...
        // [FORK] Apply env-var redirects on top of the channel config.
        if let Some(url) = env_overrides::rudderstack_url_override() {
            dest.root_url = url;
        }
        if let Some(key) = env_overrides::rudderstack_non_ugc_write_key_override() {
            dest.write_key = key;
        }
        dest
    }
    ...
    pub fn rudderstack_ugc_destination() -> RudderStackDestination {
        ...
        if let Some(key) = env_overrides::rudderstack_ugc_write_key_override() {
            dest.write_key = key;
        }
        dest
    }
    ...
    pub fn sentry_url() -> Cow<'static, str> {
        // [FORK] Kill-switch returns empty DSN, which makes the Sentry SDK a no-op.
        if env_overrides::telemetry_disabled() {
            return Cow::Borrowed("");
        }
        // [FORK] Allow `WARP_SENTRY_DSN` to redirect crash reports.
        if let Some(url) = env_overrides::sentry_url_override() {
            return url;
        }
        CHANNEL_STATE
            .lock()
