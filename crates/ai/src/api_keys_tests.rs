...
// ── [FORK] Base-URL override tests ──────────────────────────────────────────

#[test]
fn fork_defaults_are_none() {
    let keys = ApiKeys::default();
...
#[test]
fn fork_effective_openai_base_url_uses_override() {
    let keys = ApiKeys {
...
#[test]
fn fork_effective_anthropic_base_url_falls_back_to_canonical() {
    let keys = ApiKeys::default();
    assert_eq!(
...
#[test]
fn fork_effective_anthropic_base_url_uses_override() {
    let keys = ApiKeys {
...
#[test]
fn fork_deserializes_legacy_json_without_url_fields() {
    let legacy = r#"{
        "google": null,
        "anthropic": "ant-key",
        "openai": "oai-key",
...
#[test]
fn fork_serialization_omits_unset_url_fields() {
    let keys = ApiKeys::default();
    let json = serde_json::to_string(&keys).unwrap();
...
#[test]
fn fork_serialization_includes_set_url_fields() {
    let keys = ApiKeys {
        openai_base_url: Some("http://localhost:11434/v1".into()),
        ..Default::default()
    };
