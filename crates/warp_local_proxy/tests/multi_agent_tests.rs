...
#[allow(deprecated)]
fn message_tool_result_coverage(result: &message::tool_call_result::Result) -> &'static str {
...
fn decode_sse_events(body: &str) -> Vec<warp_multi_agent_api::ResponseEvent> {
    body.lines()
        ...
        .map(|l| {
            ...
            warp_multi_agent_api::ResponseEvent::decode(bytes.as_slice()).expect("valid protobuf")
        })
        .collect()
}
...
fn finished_done_event_round_trips() {
    ...
    let decoded = warp_multi_agent_api::ResponseEvent::decode(
        URL_SAFE.decode(URL_SAFE.encode(&bytes)).unwrap().as_slice(),
    )
    .unwrap();

    assert!(matches!(
        decoded.r#type,
        Some(response_event::Type::Finished(
            response_event::StreamFinished {
...
fn make_tool_result_request(
    ...
    use warp_multi_agent_api::{
        request, run_shell_command_result, RunShellCommandResult, ShellCommandFinished,
    };
    warp_multi_agent_api::Request {
        input: Some(request::Input {
...
fn extract_query(req: &warp_multi_agent_api::Request) -> Option<String> {
    ...
    if let warp_multi_agent_api::request::input::Type::UserInputs(ui) = input {
        for u in &ui.inputs {
            if let Some(
                warp_multi_agent_api::request::input::user_inputs::user_input::Input::UserQuery(q),
            ) = u.input.as_ref()
            {
...
fn extract_results(req: &warp_multi_agent_api::Request) -> Vec<String> {
    let mut out = Vec::new();
    let Some(input) = req.input.as_ref().and_then(|i| i.r#type.as_ref()) else {
        return out;
    };
    if let warp_multi_agent_api::request::input::Type::UserInputs(ui) = input {
        for u in &ui.inputs {
...
fn run_shell_command_round_trips() {
    ...
    // Proto → OpenAI name
    assert!(
        matches!(&tc.tool, Some(message::tool_call::Tool::RunShellCommand(c)) if c.command == "echo hi")
    );
    ...
    assert!(
        matches!(&back.tool, Some(message::tool_call::Tool::RunShellCommand(c)) if c.command == "echo hi")
    );
}
...
fn read_files_proto_has_correct_name() {
    let tc = message::ToolCall {
        ...
        tool: Some(message::tool_call::Tool::ReadFiles(
            message::tool_call::ReadFiles {
                files: vec![message::tool_call::read_files::File {
                    name: "foo.rs".into(),
                    line_ranges: vec![],
                }],
            },
    ...
    assert!(matches!(
        &tc.tool,
        Some(message::tool_call::Tool::ReadFiles(_))
    ));
}
        ...
        "read_files" => {
            let files = args["files"]
                ...
                .map(|a| {
                    a.iter()
                        .filter_map(|f| {
                            Some(message::tool_call::read_files::File {
...
fn permission_denied_to_text() {
    use warp_multi_agent_api::{request, run_shell_command_result, RunShellCommandResult};
    let tcr = request::input::ToolCallResult {
        tool_call_id: "tc-2".into(),
        result: Some(request::input::tool_call_result::Result::RunShellCommand(
            RunShellCommandResult {
...
fn grep_result_to_text() {
    use warp_multi_agent_api::{grep_result, request, GrepResult};
    let tcr = request::input::ToolCallResult {
...
fn tool_call_event_structure() {
    use warp_multi_agent_api::{client_action, response_event};
    ...
    match &decoded.r#type {
        Some(response_event::Type::ClientActions(ca)) => {
            ...
            if let Some(client_action::Action::AddMessagesToTask(amt)) = &ca.actions[0].action {
                ...
                assert!(
                    matches!(&amt.messages[0].message, Some(message::Message::ToolCall(tc)) if tc.tool_call_id == "call-1")
                );
            } else {
...
fn user_query_request_round_trips() {
    ...
    let decoded = warp_multi_agent_api::Request::decode(bytes.as_slice()).unwrap();
    assert_eq!(
        extract_query(&decoded).as_deref(),
